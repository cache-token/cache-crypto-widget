...
describe("Wrapper contract test", function () {
  ...
  beforeEach("deploy contracts", async () => {
    ...
    await cacheGold
      .connect(cgtHolder)
      .transfer(wrapper.address, 200 * 10 ** (await cacheGold.decimals()));
  });
  ...
  it("3. tokens can be swapped for CGT", async function () {
    ...
    const initialCGTBalance = (
      await cacheGold.balanceOf(await xTokenHolder.getAddress())
    ).toNumber();
    const initialUSDCBalance = (
      await USDC.balanceOf(wrapper.address)
    ).toNumber();

    await xToken
      .connect(xTokenHolder)
      ...
      wrapper
        ...
        .swapTokensForCGT(
          ...
          ethers.utils
            ...
            .div(10 ** (18 - (await xToken.decimals()))),
          0,
          0
        )
  ...
  it("4. owner can claim USDC tokens from the contract", async function () {
    ...
    expect(finalUSDCBalance - initialUSDCBalance).to.equal(USDCAmountCollected);
  });
  ...
  it("6. swap reverts for invalid params", async function () {
    ...
    const amount1 = await xToken.balanceOf(
      ContractAddresses[testNetwork].XToken_Holder
    );
    await expect(
      wrapper.connect(xTokenHolder).swapTokensForCGT(
        ContractAddresses[testNetwork].XToken_Address,
        3000,
        ethers.utils
